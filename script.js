...
var bogEnemy = [{
  ...
  loot: [{
    ...
    description: "The radiant eye of a bug.",
    value: 2,
    quantity: 1
  }, {
    name: "Bug Wing",
    description: "A transparent bug flapper.",
    value: 1,
    quantity: 1
  }, {
    name: "Bug Stinger",
    description: "Pointy end of the butt.",
    value: 1,
    quantity: 1
  }]
}, {
  ...
  loot: [{
    name: "Snake Legs",
    description: "The microscopic atrophied legs of a snake.",
    value: 2,
    quantity: 1
  }, {
    name: "Snake Venom",
    description: "The bog natives call it juice.",
    value: 2,
    quantity: 1
  }, {
    name: "Half-Digested Bug",
    description: "You killed the snake before it finished eating.",
    value: 1,
    quantity: 1
  }]
}, {
  ...
  loot: [{
    name: "Crab Meat",
    description: "Bugs are very attracted to its scent.",
    value: 1,
    quantity: 1
  }, {
    name: "Broken Crab Leg",
    description: "More useless than the snake leg.",
    value: 1,
    quantity: 1
  }, {
    name: "Crab Eggs",
    description: "A bundle of slimy crustacean eggs.",
    value: 3,
    quantity: 1
  }]
}]
...
var bogItem = [{
  slot: "primary",
  name: "Razor Sharp Bug Leg",
  damage: 5,
  delay: 35,
  description: "Could be used as a crude weapon.",
  value: 1,
  quantity: 1
}, {
  slot: "none",
...
function bogMerchant(input) {
  if (input.substring(0, 3) == "buy") {
    ...
    for (var i = 0; i < bogMerchantItem.length; i++) {
      ...
      var inv_spot = inventory.map(function(e) {
        return e.name;
      }).indexOf(bogMerchantItem[i].name)
      ...
      if (input.substring(4) == merch_item && player.currency >= bogMerchantItem[i].price) {
        ...
        if (bogMerchantItem[i].quantity > 1) {
          if (inventory.filter(function(e) {
              return e.name == bogMerchantItem[i].name;
            }).length == 0) {
...
function refreshStatWindow() {
  ...
  if (player.hitpoints < 0) {
    ...
    $("#moves").append("Moves: " + player.moves)
...
function lootDrop(enemy) {
  ...
  if (inventory.filter(function(e) {
    ...
    inventory[inventory.map(function(e) {
      return e.name;
    }).indexOf(item_drop.name)].quantity += 1
    $("<p>You received " + item_drop.name + " and " + money + " platinum from the fallen " + enemy.name + ".</p>").insertBefore("#placeholder")
    $("#mainConsole").scrollTop($("#mainConsole")[0].scrollHeight)
  }
}
...
$(document).ready(function() {
  ...
  $("form").submit(function() {
    ...
    if (player.hitpoints > 0) {
      ...
      //look eBogCoast
      else if (input == "look" && currentArea == "eBogCoast" && inCombat == false) {
        $(eBogCoastInfo).insertBefore("#placeholder")
      }
      ...
      //in combat and cannot move condition
      ...
      //exhausted movement options condition
      else if ((input == "west" || input == "w" || input == "east" || input == "e" || input == "north" || input == "n" || input == "south" || input == "s")) {
        $("<p> > " + input + "</p>").insertBefore("#placeholder")
